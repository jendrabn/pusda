      ...
      <div class="card-body">
        <form action="{{ route('admin.uraian.' . $crudRoutePart . '.store', $tabel->id) }}" method="POST">
          ...
          <div class="form-group">
            ...
            <select class="form-control select2 @error('parent_id') is-invalid @enderror" id="parent_id" name="parent_id" style="width: 100%">
          ...
          <div class="form-group">
            ...
            <input class="form-control @error('uraian') is-invalid @enderror" name="uraian" type="text" value="{{ old('uraian') }}">
            @error('uraian')
            <span class="error invalid-feedback">{{ $message }}</span>
            @enderror
          </div>
                  ...
                  <li> {{ $child->nama_menu }}
                    <ul>
                      ...
                      <li @if (isset($tabel) && $tabel->id === $child->id) data-jstree='{ "selected" : true }' @endif>
                        <a href="{{ route('admin.uraian.' . $crudRoutePart . '.index', $child->id) }}">{{ $child->nama_menu }}</a>
                      </li>
                      @endforeach
                      @endif
                    </ul>
    ...
    <table class="table-bordered table-striped table-hover ajaxTable datatable datatable-uraian table">
      ...
      <tbody>
        ...
        <tr>
          <td></td>
          <td>
            <a class="btn btn-xs btn-info" href="{{ route('admin.uraian.' . $crudRoutePart . '.edit', [$tabel->id, $item->id]) }}">
              Edit
            </a>
            <form style="display: inline-block;" action="{{ route('admin.uraian.' . $crudRoutePart . '.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are You Sure?');">
              @method('DELETE')
              @csrf
