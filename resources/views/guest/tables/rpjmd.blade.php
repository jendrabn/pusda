  ...
  <div class="container">
    <div class="card">
      <div class="card-header bg-white card-header__lg">
        <h4 class="card-header__title">Tabel Uraian "{{ $tabelRpjmd->menu_name }}" RPJMD</h4>
      </div>
      <div class="card-body">
        <div class="action">
          @include('guest.partials.button-export', ['id' => $tabelRpjmd->id, 'resourceName' => 'rpjmd'])
        </div>
          ...
          <table class="table table-bordered table-hover">
            ...
            <tbody>
              @foreach ($uraianRpjmd as $i => $uraian)
                ...
                @foreach ($uraian->childs as $child)
                  <tr>
                    ...
                    <td class="text-center">
                      <button type="button" class="btn btn-primary btn-sm btn-chart" data-id="{{ $child->id }}">Grafik
                      </button>
                    </td>
                ...
                @endforeach
              @endforeach
              @include('guest.partials.fitur', ['fitur' => $fiturRpjmd, 'colspan' => 6 + count($years)])
            </tbody>
          </table>
        </div>
      </div>
