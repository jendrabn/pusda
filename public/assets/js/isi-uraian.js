...
function fillFormEdit(data) {
  ...
  const { uraian, satuan, uraian_id, uraian_parent_id, isi, ketersediaan_data } = data;
  ...
  if (ketersediaan_data && ketersediaan_data != undefined) {
    modal.find('select[name=ketersediaan_data]').val(ketersediaan_data ? 1 : 0);
  }
  ...
  $('#isi-uraian-table').on('click', 'tbody .btn-grafik', function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        const years = isi.map(function (value) {
          return value.tahun;
        }).reverse();

        const isiUraian = isi.map(function (value) {
          return value.isi;
        }).reverse();

        $('#chart-isi-uraian').remove();
        $('#chart-container').append('<canvas id="chart-isi-uraian" width="100%" height="100%"></canvas>');
